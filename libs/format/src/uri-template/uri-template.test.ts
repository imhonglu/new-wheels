...
describe("URITemplate", () => {
	test("should parse a valid URITemplate string", () => {
		...
		expect(JSON.stringify(URITemplate.parse("www{.dom*}"))).toBe(
			'"www{.dom*}"',
		);
	});
	...
	test("should expand template with dot operator and string value", () => {
		...
		expect(
			URITemplate.expand("find{?year*}", { year: [1965, 2000, 2012] }),
		).toBe("find?year=1965&year=2000&year=2012");
	});
	...
	test("should handle missing values", () => {
		...
		expect(URITemplate.expand(template, { id: "123" })).toBe(
			"/users/123/posts/",
		);
	});
