...
공개 심볼의 설명, 매개변수, 반환값과 예제는 소스 TSDoc을 기준으로 관리합니다. API 레퍼런스는 이 소스에서 생성해 저장소에 커밋하고 직접 수정하지 않습니다.
...
- 공개 API 변경은 생성 레퍼런스 갱신과 diff 검토를 동반합니다.
- 생성 도구의 출력 차이가 리뷰 노이즈를 만들 수 있습니다.
