...
2. 제목은 [이슈 제목](#이슈-제목)에 따라 대상과 달성할 결과가 드러나는 명사형으로 작성합니다.
...
제목은 목록과 검색 결과에서 작업을 구분하는 식별자입니다. 배경·구현 방법과 완료 조건을 모두 요약하지 않고, 제목만 읽어도 무엇이 어떻게 달라질지 판단하는 데 필요한 최소 정보만 담습니다.

- 한국어를 기본으로 `<대상> <달성할 결과>` 순서의 간결한 명사형으로 작성합니다. 예: `enum 스키마 리터럴 유니언 추론 수정`.
- API, 패키지와 설정 이름은 원래 표기와 대소문자를 유지합니다. GitHub 제목에서는 backtick 같은 Markdown 장식을 사용하지 않습니다.
- 결과는 `추가`, `수정`, `제거`, `분리`, `통일`, `문서화`처럼 완료 여부를 판단할 수 있는 말로 끝냅니다. `작업`, `대응`, `개선`이나 `정리`만으로 결과를 대신하지 않습니다.
- 하나의 독립적으로 완료할 결과만 담습니다. 함께 적용해야 의미가 있는 변화는 연결할 수 있지만, 별도로 승인하거나 종료할 수 있으면 이슈를 나눕니다.
- `[버그]`, `WIP` 같은 상태·유형 접두사, 이슈 번호, 담당자, 우선순위와 package label을 제목에 반복하지 않습니다. 마침표와 장식용 기호도 붙이지 않습니다.
- 조사하면서 실제 결과나 대상이 달라지면 구현 전에 제목을 갱신합니다. 이슈 번호와 기존 링크는 유지되므로, 검색성과 정확성이 좋아지는 경우에는 닫힌 이슈도 제목을 바로잡을 수 있습니다.

| 피할 제목 | 권장 제목 | 이유 |
| --- | --- | --- |
| `타입 추론 작업` | `enum 스키마 리터럴 유니언 추론 수정` | 대상과 수정할 결과를 드러냅니다. |
| `[CI] 배포 개선` | `모노레포 패키지 배포 CI/CD 구성` | label과 상태 표현 대신 완료 상태를 설명합니다. |
| `createSchema helper 추가` | `스키마 유니언·기본값 추론 개선과 createSchemaClass 추가` | 실제 API 이름과 구체적인 개선 대상을 기록합니다. |

제목이 길어진다면 정보를 삭제하기 전에 결과가 둘 이상 섞였는지 확인합니다. 재현 조건, 제외 범위와 구현 상세는 제목에 압축하지 않고 본문의 해당 필드에 둡니다.
